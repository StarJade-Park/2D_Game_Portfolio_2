...
public:
	...
	unsigned int GetValue(const unsigned int nMaxValue)
	{
	...
	float GetFloatValue(const float fMinValue, const float fMaxValue)
	{
		return fMinValue + GetFloatValue( ) * (fMaxValue - fMinValue);
	}

	float GetFloatValue(const float fMaxValue)
	{
