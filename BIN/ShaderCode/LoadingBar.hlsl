...
cbuffer BAR : register(b3)
{
    float4 BAR;
}

VTX_OUT VS_LOADBAR(VTX_IN _in)
{
...
PIX_OUT PS_LOADBAR(VTX_OUT _in)
{
    PIX_OUT Out = (PIX_OUT) 0.0f;
    Out.vOutColor = Tex.Sample(Smp, _in.vUv);

    if (_in.vUv.x <= 1.0f - BAR.x)
    {
        Out.vOutColor.r = COLOR.r;
        Out.vOutColor.g = COLOR.g;
        Out.vOutColor.b = COLOR.b;
    }

    if (_in.vUv.y <= 1.0f - BAR.y)
    {
        Out.vOutColor.r = COLOR.r;
        Out.vOutColor.g = COLOR.g;
        Out.vOutColor.b = COLOR.b;
